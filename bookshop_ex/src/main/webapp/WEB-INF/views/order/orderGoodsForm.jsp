...
<head>
	...
	<script>
		...
		//상품 주문 페이지가 표시되면 주문자의 휴대번호와 유선 전화번호를 셀렉트 박스에 표시
		...
		function reset_all(){
			...
			var e_roadAddress = document.getElementById("raodAddress");
			var e_jibunAddress = document.getElementById("jibunAddress");
			var d_namujiAddress = document.getElementById("namujiAddress");
		...
		function fn_show_order_detail(){
			...
			for(var i = 0; i< r_pay_method.length; i++){
				if(r_pay_method[i].checked == true){
					...
					if(pay_method == "신용카드"){
						...
						pay_orderer_hp_num = i_pay_order_tel1.value+"-"+i_pay_tel2.value+"-"+i_pay_tel3.value;
						pay_method += "<Br>"+"결제휴대폰번호:"+pay_order_hp_num;
						card_com_name = "해당없음";
						card_pay_month = "해당없음";
					}
					break;
			...
			delivery_address="우편번호:"+i_zicode.value+"<br>"+"도로명 주소 : "+i_roadAddress.value+"<br>"+
					...
					<c:forEach var="item" items="${myOrderList}">
						<td class="goods_image">
							<a href="${contextPath}/goods/goodsDetail.do?goods_id=${item.goods_id}">
								<img width="75" alt="" src="${contextPath}/thumnails.do?goods_id=${item.goods_id}&fileName=${item.goods_fileName}">
						...
						<td>
							<h2>
								<a href="${pageContext.request.contextPath}/goods/goods.do?command=good_detail&goods_id=${item.goods_id}">${item.goods_title}</a>
						...
						<td><h2>${item.goods_sales_price}원 (10%할인)</h2>
						<td><h2>0원</h2>
						<td><h2>${1500 * item.order_goods_qty}원</h2>
						<td>
							<h2>${item.goods_sales_price * item.order_godos_qty}원</h2>
			...
			<table>
				<tbody>
					<tr class="dot_line">
						<td><h2>이름</h2></td>
						<td>
							<input type="text" value="${orderer.member.name}" size="15"/>
						</td>
					</tr>
					<tr class="dot_line">
						<td><h2>핸드폰</h2></td>
