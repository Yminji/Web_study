...
<head>
<meta charset="UTF-8">
<!-- Daum에서 제공하는 주소 검색을 사용하기 위해 API 포함 -->
...
<script>
	function execDaumPostcode(){
		new daum.Postcode({
			oncomplete: function(data){
				...
				var extrRoadAddr = ''; //도로명 조합형 주소 변수
				...
				//도록명, 지번 조합형 주소가 있을 경우, 괄호까지 추가한 최종 문자열을 만듦
				if(extraRoadAddr !== ''){
				...
				document.getElementById('zipcode').value = data.zonecode; //5자리 새우편 번호 사용
				document.getElementById('roadAddress').value = fullRoadAddr;
				document.getElementById('jibunAddress').value = data.jibunAddress;
				
				//사용자가 '선택 안함'을 클릭한 경우, 예상 주소라는 표시를 해준다.
				if(data.autoRoadAddress){
					//예상되는 도로명 주소에 조합형 주소 추가함
					var expRoadAddr = data.autoRoadAddress + extraRoadAddr;
					document.getElementById('guid').innerHTML = '(예상 도로명 주소 : '+ expRoadAddr + ')';
				}else if(data.autoJibunAddress){
					var exJibunAddr = data.autoJibunAddress;
					document.getElementById('guid').innerHTML = '(예상 지번 주소: ' +expJibunAddr+')';
				}else{
	...
	function fn_overlapped(){
		...
		$.ajax({
			type:"post",
			asyn:false,
			url:"${contextPath}/membere/overlapped.do",
			dataType:"text",
			data:{id:_id},
			success:functioin(data, textStatus){
				if(data=='false'){
				...
				<tbody>
					...
					<tr class="dot_line">
						...
						<td><input type="radio" name="member_gender" value="102"/>
							여성<span style="padding_left:120px"></span>
					...
					<tr class="dot_line">
						<td class="fixed_join">이메일<br>(e_mail)</td>
